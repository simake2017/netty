...
/**
 * Promise接口继承自Future ，Promise可以写，也是异步任务
 *  promise 和 future 主要用来作为一个 （异步）任务的执行结果
 * @param <V>
 */
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    ...
    private final EventExecutor executor; //用于支持事件执行，用executor执行任务
    /**
    ...
    public boolean setUncancellable() {
        //成功将result字段的值改为UNCANCELLABLE（Singal类型对象）
    ...
    /**
     * 设置 result的值为  Success
     * @param result
     * @return
     */
    private boolean setSuccess0(V result) {
        /**
         * 当 result 为 null的时候 设置为 SUCCESS
         */
