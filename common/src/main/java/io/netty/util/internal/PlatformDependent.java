...
public final class PlatformDependent {
    ...
    private static final boolean IS_WINDOWS = isWindows0(); //是否是window系统
    private static final boolean MAYBE_SUPER_USER;

    private static final boolean CAN_ENABLE_TCP_NODELAY_BY_DEFAULT = !isAndroid();//是否可以使用tcp不延迟，android系统不可以

    private static final boolean HAS_UNSAFE = hasUnsafe0();  //  是否存在 unsafe 类
    private static final boolean DIRECT_BUFFER_PREFERRED =
            HAS_UNSAFE && !SystemPropertyUtil.getBoolean("io.netty.noPreferDirect", false);   // 该参数判断是否 不使用 直接内存
    ...
    private static long maxDirectMemory0() {
        ...
        try {
            ...
            /**
             * 获取输入参数，找到MaxDirectMemorySize最大直接内存参数
             * 最大直接内存参数
             */
