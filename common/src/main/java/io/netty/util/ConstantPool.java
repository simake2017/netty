...
这里的ConstantPool本身是一个泛型类，创建对象时需要一个泛型对象
 */
//ConstantPool使用的参数化泛型对象是T进行编译期的约束使用，将该类型对象放入到map中作为，泛型类型
public abstract class ConstantPool<T extends Constant<T>> {
    ...
    private T createOrThrow(String name) {
        ...
        if (constant == null) {
            final T tempConstant = newConstant(nextId(), name);
            constant = constants.putIfAbsent(name, tempConstant);// --> 不存在 创建，存在，返回现存的值
            if (constant == null) {
                return tempConstant;
            }
        }
