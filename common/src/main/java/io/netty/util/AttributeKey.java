...
/**
 * attributeKey是自定义的属性key，以该类对象作为一个Value存放在ConstantPool中
 * 这里的泛型对象主要是进行约束使用的，就是一个参数化类型，在编译期间进行类型约束，在编译期间限制类型统一
 */
public final class AttributeKey<T> extends AbstractConstant<AttributeKey<T>> {

    //AttributeKey是继承自Constant的，这里作为放入ConstantPool中的参数化约束对象
    ...
    /**
     * 根据name获取对应的AttributeKey，如果没有，就创建一个
     */
