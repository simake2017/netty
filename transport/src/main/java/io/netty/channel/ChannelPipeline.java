 ...
 *
 *
    在handler中调用事件传播方法，传播事件，事件会继续按照后续的调用顺序传播事件

    这里是入站 触发 事件    由inboundinvoker进行触发，触发后 ，由pipeline中的

    pipeline 和 handlerContext都可以 触发相应的 节点事件

    触发后，会由pipeline中的节点进行 相应 的 入站处理 或者 出站处理

 * <li>Inbound event propagation methods: inbound事件
 *     <ul>
 *     <li>{@link ChannelHandlerContext#fireChannelRegistered()}</li>注册
 *     <li>{@link ChannelHandlerContext#fireChannelActive()}</li> 激活
 *     <li>{@link ChannelHandlerContext#fireChannelRead(Object)}</li> 读
 *     <li>{@link ChannelHandlerContext#fireChannelReadComplete()}</li> 读完成
 *     <li>{@link ChannelHandlerContext#fireExceptionCaught(Throwable)}</li> 异常
 *     <li>{@link ChannelHandlerContext#fireUserEventTriggered(Object)}</li> 用户事件触发
 *     <li>{@link ChannelHandlerContext#fireChannelWritabilityChanged()}</li>
 *     <li>{@link ChannelHandlerContext#fireChannelInactive()}</li>
 *     <li>{@link ChannelHandlerContext#fireChannelUnregistered()}</li>
 *     </ul>
 * </li>
 * <li>Outbound event propagation methods: 出站事件
 *     <ul>
 *     <li>{@link ChannelHandlerContext#bind(SocketAddress, ChannelPromise)}</li> 绑定
 *     <li>{@link ChannelHandlerContext#connect(SocketAddress, SocketAddress, ChannelPromise)}</li> 连接
 *     <li>{@link ChannelHandlerContext#write(Object, ChannelPromise)}</li> 写
 *     <li>{@link ChannelHandlerContext#flush()}</li>
 *     <li>{@link ChannelHandlerContext#read()}</li> 读
...
/*
    ...
    ChannelPipeline是一个类似于Linklist的结构 ，而且还继承了Iterable接口，可以进行迭代

    pipeline用于触发 事件   handlerAdapter用于处理事件

    继承了 inBoundInvoker与outBoundInvoker接口 可以触发相应的事件

    invoke

 */
/*
    pipeline的入站事件从头开始  head节点开始
                出站事件从tail节点 开始
